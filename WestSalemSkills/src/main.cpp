...
void autonomous(void) {
  ...
  motor_group(fLDrive, bLDrive, uLDrive, fRDrive, bRDrive, uRDrive).spinFor(reverse, 0.3, sec);
 ...
 //position to score in long goal 1
  ...
  wait(2.5, sec); //score in long goal 1
  scraper.set(true);
  intakeupper.setVelocity(10, pct);
  intakelower.setVelocity(100, pct);
  intakelower.spinFor(fwd, 676767, rev, false);
  ...
  wait(1, sec);
  drive(15);
  wait(0.5, sec); //remove from loader 2
  drive(-110);
  turn(183);
  drive(-110);
  intakeupper.setVelocity(100, pct);
